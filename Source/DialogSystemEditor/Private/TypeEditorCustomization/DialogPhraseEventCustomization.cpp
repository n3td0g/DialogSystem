...
#include "Dialog/DialogPhraseEvent.h"
...
void FDialogPhraseEventCustomization::CustomizeChildren(TSharedRef<IPropertyHandle> InStructPropertyHandle, IDetailChildrenBuilder& StructBuilder, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	uint32 i = 0;
	for (TFieldIterator<FProperty> PropIt(func); PropIt && (PropIt->PropertyFlags & CPF_Parm); ++PropIt, ++i)
	{
...
FReply FDialogPhraseEventCustomization::OnTitleClick()
{
	...
	if (Property_CallType == EDialogPhraseEventCallType::DialogScript && Property_OwnerNode != NULL)
	{
		...
		if (sc != NULL)
		{
			...
			{
				AssetEditorSubsystem->OpenEditorsForAssets(TArray<FName>({ *sc->GetPathName() }));
			}
		}
	}
	else if (Property_ObjectClass != NULL)
	{
		if (UAssetEditorSubsystem* AssetEditorSubsystem = GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()) 
		{
			AssetEditorSubsystem->OpenEditorsForAssets(TArray<FName>({ *Property_ObjectClass->GetPathName() }));
		}		
	}
